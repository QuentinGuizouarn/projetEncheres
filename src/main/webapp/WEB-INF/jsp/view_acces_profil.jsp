...
.property-image {
	...
	transition: all 0.4s cubic-bezier(0.645, 0.045, 0.355, 1);
	background-image:
		url('https://www.cdiscount.com/pdt2/5/0/8/1/700x700/auc3937114274508/rw/kit-autocollants-clavier-azerty-francais.jpg');
		...
		<div class="container-fluid">
			...
			<div>
				<a href="<%=request.getContextPath()%>/nouvelle_vente">Vendre un
					article</a> <a
					href="<%=request.getContextPath()%>/view_profil?user=<%=u != null ? u.getIdUtilisateur() : null%>">Mon
					profil</a> <a href="<%=request.getContextPath()%>/DeconnexionServlet">Deconnexion</a>
			</div>
		</div>
	</nav>
	<main>
		<form action="<%=request.getContextPath()%>/AccesProfilServlet"
			method="POST">
		...
		%> --%>
			<div>
				...
				<br> <br> <label for="categorie"> Catégorie : </label> <select
					name="categorie" id="categorie" onChange="afficherCardsByLibelle()">
			...
			<div id="visibleAchat">
				...
				<label class="checkbox" for="ouverte"> <input
					type="checkbox" value="ouverte" id="ouverte" name="ouverte">
					Enchères ouvertes
				</label><br> <label class="checkbox" for="mesEncheres"> 
				<input
					...
					type="checkbox" value="remportees" id="remportees"
					name="remportees"> Mes enchères remportées
				</label>
				<div id="visibleVente"
					style="margin-left: 265px; margin-top: -71px;">
					<label class="checkbox" for="enCours"> <input
						...
						type="checkbox" value="terminee" id="terminee" name="terminee">
						Ventes terminées
					</label>
				</div>
				<br>
		...
		<div id="tableauListe">
			<div class="center" style="margin-top: -10%">
				...
				<div class="property-card">
					<a
						href="<%=request.getContextPath()%>/detail_vente?article=<%=article != null ? article.getIdArticle() : null%>">
						...
						<div class="property-description">
							...
							<p
								<%=article.getPrixVente() != 0 ? article.getPrixVente() : article.getPrixInitial()%>>
								points</p>
...
<script type="text/javascript">
	...
	/* function afficherCardsAll() {
		...
		if (idTable.value == "toute") {
			document.getElementById("tableauListe").style.display = "block";
		} else {
