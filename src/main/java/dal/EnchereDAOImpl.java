...
public class EnchereDAOImpl implements EnchereDAO {
	...
	private static final String SELECT_MAX_BY_ARTICLE = "SELECT idEnchere = MAX(E.idEnchere), dateEnchere = E.date, E.montant,"
			...
			+ " FROM ENCHERES E"
			+ " INNER JOIN UTILISATEURS UV ON UV.idUtilisateur = E.idUtilisateur"
			+ " INNER JOIN ARTICLES_VENDUS A ON A.idArticle = E.idArticle"
			+ " INNER JOIN UTILISATEURS UA ON UA.idUtilisateur = A.idUtilisateur"
			+ " INNER JOIN CATEGORIES C ON C.idCategorie = A.idCategorie"
			+ " WHERE E.idArticle = ?"
			+ " GROUP BY E.date, E.montant, A.idArticle, A.nom, A.description, C.libelle, A.prixInitial, A.prixVente,"
			+ "	A.dateFin, A.rue, A.codePostal, A.ville, A.etat, UA.idUtilisateur, UA.pseudo, UV.idUtilisateur, UV.pseudo";
	...
	public Enchere selectMaxByArticle(int idArticle) throws SQLException {
		...
		if (rs.next()) {
			av = new ArticleVendu( rs.getInt(idArticle), rs.getInt("prixInitial"), rs.getInt("prixVente"), rs.getString("nom"), 
				...
				rs.getString("pseudoVendeur"), rs.getString("telephoneVendeur") ), 
				new Categorie( rs.getInt("idCategorie"), rs.getString("libelle") ) );
